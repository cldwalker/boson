...
* Libraries
  * can be written in non-dsl ruby which allows for easy testing and use independent of boson (Boson::FileLibrary).
  ...
  * are social as a user can install them from a url and then customize command names, aliases and options without touching
    the original library.
* Commands
  * have powerful options (Boson::OptionParser): global options, rendering options and local options
    specific to the command (Boson::OptionCommand).
  * can have any view associated to it (via Hirb) without adding view code to the command's method.
    These views can be toggled on and manipulated via global options (Boson::View and Boson::OptionCommand).
  * can have post-command commands accessible as options which change the returned value. Default post-command commands include
    commands to search and sort an array of any objects (Boson::Scientist).
